...
class Store extends React.Component {
  ...
  showProductDetails = (productID) => {
    ...
    fetch('http://localhost:4200/api/store/get-product-details', {
      method: 'POST',
      headers: {
  ...
  getAllProducts = () => {
    ...
    fetch('http://localhost:4200/api/store/get-products', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      }
      ...
      <Tabs activeKey={this.state.activeKey} onChange={this.onTabChange}>
        ...
        <Tabs.TabPane tab="Add Product" key="1">
          <Spin size="large" tip="Loading..." spinning={this.state.loading} style={{minHeight: '200px'}}>
            <Product changeTab={this.showAllProducts} productDetails={this.state.productDetails} form={this.state.form}/>
          </Spin>
        </Tabs.TabPane>

        <Tabs.TabPane tab="View Products" key="2">
          <Spin size="large" tip="Loading..." spinning={this.state.loading} style={{minHeight: '200px'}}>
            <AllProducts changeTab={this.showProductDetails} products={this.state.allProducts} deteleProduct={this.deleteProduct}/>
          </Spin>
        </Tabs.TabPane>
